...
class ToursList implements ShortcodeInterface
{
            ...
            'params'                    => array_merge(
                array(
                    ...
                    array(
                        ...
                        'description' => esc_html__('Enter custom image dimensions. Enter image size in pixels: 200x100 (Width x Height)',
                            'setsail-tours'),
                    ...
                    array(
                        ...
                        'description' => esc_html__('Number of words (Default value is empty - hidden excerpt)',
                            'setsail-tours'),
                        'dependency'  => array(
                            'element' => 'tour_type',
                            'value'   => array('standard', 'gallery-with-description')
                        )
                    ),
                    array(
                        'type'        => 'dropdown',
    ...
    public function render($atts, $content = null)
    {
        ...
        $params['load_more_text']         = empty($params['load_more_text']) ? esc_html__('Load More',
            'setsail-tours') : $params['load_more_text'];
        $params['display_load_more_data'] = (int)$params['number'] == $params['number'] && $params['number'] > 0;

        return setsail_tours_get_tour_module_template_part('tours-list/templates/tours-list-holder', 'tours',
            'shortcodes', '', $params);
    }
    ...
    public function getMetaQueryParams($params)
    {
        ...
        if ( ! empty($params['destination'])) {
            $destination_query = new \WP_Query(array(
                'post_status' => 'published',
                'post_type'   => 'destinations',
                'name'        => esc_attr(strtolower($params['destination']))
            ));
            wp_reset_postdata();
    ...
    {
        echo setsail_tours_get_tour_module_template_part('tours-list/templates/tours-list-loop', 'tours', 'shortcodes',
            '', $params);
    }
