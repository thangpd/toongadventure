...
if(!function_exists('setsail_tours_get_tour_button')) {
    ...
    function setsail_tours_get_tour_button($params = array(), $id = '') {
        ...
        $text = isset($params['text']) && $params['text'] !== '' ? $params['text'] : esc_html__('Learn more', 'setsail-tours');
...
if(!function_exists('setsail_tours_get_tour_price_html')) {
	...
	function setsail_tours_get_tour_price_html($tour_id = null) {
		...
		<span class="<?php echo esc_attr(implode(' ', $holder_class)); ?>">
			<?php if($price) : ?>
				<span class="qodef-tours-item-price <?php echo esc_attr($price_on_discount_class);?>"><?php echo esc_html($price); ?></span>
			<?php endif; ?>
...
if(!function_exists('setsail_tours_get_checkout_page_content')) {
	function setsail_tours_get_checkout_page_content() {
		...
		echo setsail_tours_get_tour_module_template_part('checkout/checkout-content', 'tours', 'templates', '', $params);
	}
}
...
if(!function_exists('setsail_tours_get_search_page_content_html')) {
	...
	function setsail_tours_get_search_page_content_html() {
		...
		return setsail_tours_get_tour_module_template_part('search/search-content', 'tours', 'templates', '', compact('tours_list','list_classes'));
	}
}
...
if(!function_exists('setsail_tours_get_search_page_items_loop_html')) {
	 ...
	 * @return string
	 */
	function setsail_tours_get_search_page_items_loop_html($tours_list, $type = '', $text_length = null, $thumb_size = null) {
		$type = empty($type) ? setsail_tours_search()->getViewType() : $type;
		...
		return setsail_tours_get_tour_module_template_part('search/search-items-content', 'tours', 'templates', '', array(
							...
							if(setsail_tours_theme_installed() && empty($category_image)) {
								...
								if(!empty($category_icon)) { ?>
									<span class="qodef-tour-cat-item-icon">
										<?php echo setsail_select_icon_collections()->renderIcon($category_icon, $category_icon_pack); ?>
									</span>
								<?php } ?>

							<?php } else { ?>
								<span class="qodef-tour-cat-item-icon qodef-tour-cat-item-custom-image">
									<img src="<?php echo esc_url( $category_image ) ?>" alt="<?php esc_attr_e( 'term-custom-icon', 'setsail-tours' ); ?>"/>
								</span>
							<?php } ?>
