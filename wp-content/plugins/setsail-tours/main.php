...
if ( ! function_exists( 'setsail_tours_scripts' ) ) {
	...
	function setsail_tours_scripts() {
		...
		if ( function_exists( 'setsail_select_is_responsive_on' ) && setsail_select_is_responsive_on() ) {
			wp_enqueue_style( 'setsail-tours-responsive-style', plugins_url( SETSAIL_TOURS_REL_PATH . '/assets/css/tours-responsive.min.css' ), $array_deps_css_responsive );
		}
		
		wp_enqueue_style( 'nouislider', plugins_url( SETSAIL_TOURS_REL_PATH ) . '/assets/css/nouislider.min.css' );
		
		wp_enqueue_script( 'nouislider', plugins_url( SETSAIL_TOURS_REL_PATH ) . '/assets/js/modules/plugins/nouislider.min.js', array(), false, true );
		wp_enqueue_script( 'typeahead', plugins_url( SETSAIL_TOURS_REL_PATH ) . '/assets/js/modules/plugins/typeahead.bundle.min.js', array( 'jquery' ), false, true );
		wp_enqueue_script( 'bloodhound', plugins_url( SETSAIL_TOURS_REL_PATH ) . '/assets/js/modules/plugins/bloodhound.min.js', array( 'jquery' ), false, true );
		
		wp_enqueue_script( 'setsail-tours-script', plugins_url( SETSAIL_TOURS_REL_PATH . '/assets/js/tours.min.js' ), $array_deps_js, false, true );
	}
