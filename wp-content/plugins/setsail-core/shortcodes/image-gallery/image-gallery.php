...
namespace SetSailCore\CPT\Shortcodes\ImageGallery;
...
class ImageGallery implements Lib\ShortcodeInterface
{
    ...
    public function vcMap()
    {
        if (function_exists('vc_map')) {
            vc_map(
                array(
                    'name'                      => esc_html__('Image Gallery', 'setsail-core'),
                    'base'                      => $this->getBase(),
                    ...
                    'params'                    => array(
                        ...
                        array(
                            'type'        => 'dropdown',
                            'param_name'  => 'type',
                            'heading'     => esc_html__('Gallery Type', 'setsail-core'),
                            'value'       => array(
                                esc_html__('Image Grid', 'setsail-core')  => 'grid',
                                esc_html__('Toong Style', 'setsail-core') => 'toong',
                                esc_html__('Masonry', 'setsail-core')     => 'masonry',
                                esc_html__('Slider', 'setsail-core')      => 'slider',
                                esc_html__('Carousel', 'setsail-core')    => 'carousel',
                            ),
                            'save_always' => true,
                            'admin_label' => true
                        ),
                        array(
                            'type'        => 'attach_images',
                            'param_name'  => 'images',
                            'heading'     => esc_html__('Images', 'setsail-core'),
                            'description' => esc_html__('Select images from media library', 'setsail-core')
                        ),
                        array(
                            'type'        => 'textfield',
                            'param_name'  => 'image_size',
                            'heading'     => esc_html__('Image Size', 'setsail-core'),
                            'description' => esc_html__('Enter image size. Example: thumbnail, medium, large, full or other sizes defined by current theme. Alternatively enter image size in pixels: 200x100 (Width x Height). Leave empty to use "full" size',
                                'setsail-core')
                        ),
                        array(
                            'type'        => 'dropdown',
                            'param_name'  => 'enable_image_shadow',
                            'heading'     => esc_html__('Enable Image Shadow', 'setsail-core'),
                        ...
                        array(
                            ...
                            'heading'    => esc_html__('Image Behavior', 'setsail-core'),
                            'value'      => array(
                                esc_html__('None', 'setsail-core')             => '',
                                esc_html__('Open Lightbox', 'setsail-core')    => 'lightbox',
                                esc_html__('Open Custom Link', 'setsail-core') => 'custom-link',
                                esc_html__('Zoom', 'setsail-core')             => 'zoom',
                                esc_html__('Grayscale', 'setsail-core')        => 'grayscale'
                            )
                        ),
                        array(
                            'type'        => 'textarea',
                            'param_name'  => 'custom_links',
                            'heading'     => esc_html__('Custom Links', 'setsail-core'),
                            'description' => esc_html__('Delimit links by comma', 'setsail-core'),
                        ...
                        array(
                            ...
                            'group'       => esc_html__('Slider Settings', 'setsail-core')
                        ),
                        array(
                            'type'        => 'dropdown',
                        ...
                        array(
                            ...
                            'description' => esc_html__('Speed of slide animation in milliseconds. Default value is 600.',
                                'setsail-core'),
                            'dependency'  => array('element' => 'type', 'value' => array('slider', 'carousel')),
                            'group'       => esc_html__('Slider Settings', 'setsail-core')
                        ),
                        array(
                            ...
                            'heading'     => esc_html__('Enable Slider Padding', 'setsail-core'),
                            'description' => esc_html__('Padding left and right on stage (can see neighbours).',
                                'setsail-core'),
                            'value'       => array_flip(setsail_select_get_yes_no_select_array(false)),
                            'save_always' => true,
    ...
    public function render($atts, $content = null)
    {
        ...
        $html = setsail_core_get_shortcode_module_template_part('templates/' . $params['type'], 'image-gallery', '',
            $params);
