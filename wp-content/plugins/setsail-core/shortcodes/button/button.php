<?php
namespace SetSailCore\CPT\Shortcodes\Button;
...
class Button implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Button', 'setsail-core' ),
					'base'                      => $this->base,
						...
						array(
							...
							array(
								...
								'value'      => array(
									...
									esc_html__( 'Huge', 'setsail-core' )    => 'huge'
								),
							...
							array(
								'type'        => 'dropdown',
								'param_name'  => 'target',
								'heading'     => esc_html__( 'Link Target', 'setsail-core' ),
								'value'       => array_flip( setsail_select_get_link_target_array() ),
								'save_always' => true
							)
						),
						setsail_select_icon_collections()->getVCParamsArray( array(), '', true ),
						array(
							array(
								'type'       => 'colorpicker',
								'param_name' => 'color',
								'heading'    => esc_html__( 'Color', 'setsail-core' ),
								'group'      => esc_html__( 'Design Options', 'setsail-core' )
							),
							array(
								'type'       => 'colorpicker',
								'param_name' => 'hover_color',
								'heading'    => esc_html__( 'Hover Color', 'setsail-core' ),
								'group'      => esc_html__( 'Design Options', 'setsail-core' )
							),
							array(
								'type'       => 'colorpicker',
							...
							array(
								...
								'heading'    => esc_html__( 'Font Size (px)', 'setsail-core' ),
								'group'      => esc_html__( 'Design Options', 'setsail-core' )
							),
							array(
								'type'        => 'dropdown',
							...
							array(
								'type'        => 'textfield',
								'param_name'  => 'padding',
								'heading'     => esc_html__( 'Button Padding', 'setsail-core' ),
								'description' => esc_html__( 'Insert padding in format: top right bottom left (e.g. 10px 5px 10px 5px)', 'setsail-core' ),
								'dependency'  => array( 'element' => 'type', 'value' => array( 'solid', 'outline' ) ),
								'group'       => esc_html__( 'Design Options', 'setsail-core' )
							)
						)
	...
	public function render( $atts, $content = null ) {
		...
		return setsail_core_get_shortcode_module_template_part( 'templates/' . $params['html_type'], 'button', '', $params );
	}
	...
	private function getButtonStyles( $params ) {
		...
		if ( ! empty( $params['font_size'] ) ) {
			$styles[] = 'font-size: ' . setsail_select_filter_px( $params['font_size'] ) . 'px';
		}
