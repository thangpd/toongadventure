<?php
namespace SetSailCore\CPT\Shortcodes\SocialShare;
...
class SocialShare implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Social Share', 'setsail-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'value'      => array(
								esc_html__( 'List', 'setsail-core' )     => 'list',
								esc_html__( 'Dropdown', 'setsail-core' ) => 'dropdown',
								esc_html__( 'Text', 'setsail-core' )     => 'text'
							)
						),
						array(
							'type'       => 'dropdown',
							'param_name' => 'icon_type',
							'heading'    => esc_html__( 'Icons Type', 'setsail-core' ),
							'value'      => array(
								esc_html__( 'Font Awesome', 'setsail-core' ) => 'font-awesome',
								esc_html__( 'Font Elegant', 'setsail-core' ) => 'font-elegant'
							),
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'title',
							'heading'    => esc_html__( 'Social Share Title', 'setsail-core' )
						)
					)
				)
			);
	...
	public function render( $atts, $content = null ) {
		...
		$params['enable_social_share'] = setsail_select_options()->getOptionValue( 'enable_social_share' ) === 'yes';
		
		//Is social share enabled for post type
		$post_type         = str_replace( '-', '_', get_post_type() );
		$params['enabled'] = setsail_select_options()->getOptionValue( 'enable_social_share_on_' . $post_type ) === 'yes';
		...
		if ( $params['enable_social_share'] && $params['enabled'] ) {
			$html = setsail_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'social-share', '', $params );
		}
	...
	private function getSocialNetworksParams( $params ) {
		...
		foreach ( $this->socialNetworks as $net ) {
			...
			if ( setsail_select_options()->getOptionValue( 'enable_' . $net . '_share' ) == 'yes' ) {
				...
				$params['custom_icon'] = ( setsail_select_options()->getOptionValue( $net . '_icon' ) ) ? setsail_select_options()->getOptionValue( $net . '_icon' ) : '';
				
				$html = setsail_core_get_shortcode_module_template_part( 'templates/parts/network', 'social-share', '', $params );
			}
    ...
    private function getSocialNetworkShareLink($net, $image) {
        switch ($net) {
            ...
            case 'twitter':
                ...
                $twitter_via = (setsail_select_options()->getOptionValue('twitter_via') !== '') ? ' via ' . setsail_select_options()->getOptionValue('twitter_via') . ' ' : '';
                if (wp_is_mobile()) {
                    $link = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(setsail_select_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                } else {
                    $link = 'window.open(\'http://twitter.com/home?status=' . urlencode(setsail_select_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                }
                break;
            case 'google_plus':
            ...
            case 'tumblr':
                ...
                break;
            case 'pinterest':
                $link = 'popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . setsail_select_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
                break;
            case 'vk':
	...
	private function getSocialNetworkText( $net ) {
		switch ( $net ) {
			case 'facebook':
				$text = esc_html__( 'fb', 'setsail-core');
				break;
			case 'twitter':
				$text = esc_html__( 'tw', 'setsail-core');
				break;
			case 'google_plus':
				$text = esc_html__( 'g+', 'setsail-core');
				break;
			case 'linkedin':
				$text = esc_html__( 'lnkd', 'setsail-core');
				break;
			case 'tumblr':
				$text = esc_html__( 'tmb', 'setsail-core');
				break;
			case 'pinterest':
				$text = esc_html__( 'pin', 'setsail-core');
				break;
			case 'vk':
				$text = esc_html__( 'vk', 'setsail-core');
				break;
			default:
				$text = '';
		}
