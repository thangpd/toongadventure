...
namespace SetSailCore\CPT\Shortcodes\Testimonials;
...
class Testimonials implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'number',
							'heading'    => esc_html__( 'Number of Testimonials', 'setsail-core' )
						),
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
							'heading'     => esc_html__( 'Category', 'setsail-core' ),
							'description' => esc_html__( 'Enter one category slug (leave empty for showing all categories)', 'setsail-core' )
						),
						array(
							'type'        => 'dropdown',
						...
						array(
							...
							'value'       => array_flip( setsail_select_get_yes_no_select_array( false, true ) ),
							'description' => esc_html__( 'Navigation arrows will be visible on screens bigger then 1024.', 'setsail-core' ),
							'save_always' => true,
							'group'       => esc_html__( 'Slider Settings', 'setsail-core' )
						),
						array(
							...
							'heading'     => esc_html__( 'Enable Slider Pagination On Mobile Devices', 'setsail-core' ),
							'value'       => array_flip( setsail_select_get_yes_no_select_array( false, true ) ),
							'save_always' => true,
							'group'       => esc_html__( 'Slider Settings', 'setsail-core' )
						)
	...
	public function render( $atts, $content = null ) {
		...
		return setsail_core_get_cpt_shortcode_module_template_part( 'testimonials', 'testimonials', 'testimonials', '', $params );
	}
	...
	public function testimonialsCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['testimonials_category_title'] ) > 0 ) ? esc_html__( 'Category', 'setsail-core' ) . ': ' . $value['testimonials_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $testimonials_category ) ) {
				...
				if ( ! empty( $testimonials_category_title ) ) {
					$testimonials_category_title_display = esc_html__( 'Category', 'setsail-core' ) . ': ' . $testimonials_category_title;
				}
