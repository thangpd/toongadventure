...
foreach ( glob( SELECT_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( setsail_select_is_customizer_item_enabled( $module_load, 'setsail_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'setsail_select_get_icon_pack_collections' ) ) {
	function setsail_select_get_icon_pack_collections() {
		$options = apply_filters( 'setsail_select_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'setsail_select_get_icon_pack_collections' );
}

if ( ! function_exists( 'setsail_select_get_icon_pack_options' ) ) {
	function setsail_select_get_icon_pack_options() {
		$options = apply_filters( 'setsail_select_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'setsail_select_get_icon_pack_options' );
}

if ( ! function_exists( 'setsail_select_get_icon_pack_social_options' ) ) {
	function setsail_select_get_icon_pack_social_options() {
		$options = apply_filters( 'setsail_select_filter_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'setsail_select_get_icon_pack_social_options' );
}

/*
 * Class: SetSailSelectClassIconCollections
 */
class SetSailSelectClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = setsail_select_get_icon_pack_collections();
		$this->iconPackOptions       = setsail_select_get_icon_pack_options();
		$this->iconPackSocialOptions = setsail_select_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = setsail_select_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'setsail' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'setsail' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'setsail' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'setsail' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'setsail' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'setsail' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'setsail' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'setsail' )       => 'fa-lg',
			esc_html__( 'Small', 'setsail' )      => 'fa-2x',
			esc_html__( 'Medium', 'setsail' )     => 'fa-3x',
			esc_html__( 'Large', 'setsail' )      => 'fa-4x',
			esc_html__( 'Very Large', 'setsail' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iSetSailSelectInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'setsail' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'setsail' ) ] = '';
