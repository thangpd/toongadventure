...
if (!function_exists('setsail_select_include_title_types_before_load')) {
    ...
    add_action('setsail_select_action_options_map', 'setsail_select_include_title_types_before_load', 1); // 1 is set to just be before title option map init
}
...
if ( ! function_exists( 'setsail_select_include_title_types' ) ) {
	...
	add_action('setsail_select_action_options_map', 'setsail_select_include_title_types', 1); // 1 is set to just be before title option map init
}
...
if ( ! function_exists( 'setsail_select_get_title' ) ) {
	...
	function setsail_select_get_title() {
		...
		if ( $show_title_area ) {
			...
			$template_path = apply_filters( 'setsail_select_filter_title_template_path', $template_path = 'types/' . $type . '/templates/' . $type . '-title' );
			...
			$parameters = array(
				...
				'title'           => setsail_select_get_title_text(),
				'title_tag'       => $title_tag,
...
if ( ! function_exists( 'setsail_select_get_title_holder_styles' ) ) {
	...
	function setsail_select_get_title_holder_styles() {
		...
		$title_image_behavior = setsail_select_get_meta_field_intersect( 'title_area_background_image_behavior', $page_id );
...
if ( ! function_exists( 'setsail_select_get_title_area_height' ) ) {
	...
	function setsail_select_get_title_area_height() {
		...
		$title_height      = ! empty( $title_height_meta ) ? intval( $title_height_meta ) : apply_filters( 'setsail_select_filter_title_area_default_height_value', 240 );
...
if ( ! function_exists( 'setsail_select_get_subtitle_styles' ) ) {
	...
	function setsail_select_get_subtitle_styles() {
		...
		if ( $side_padding !== '' ) {
			if ( setsail_select_string_ends_with( $side_padding, '%' ) || setsail_select_string_ends_with( $side_padding, 'px' ) ) {
